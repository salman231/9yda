...
class Repository implements ChatRepositoryInterface
{
    ...
    public function save(\Amasty\Rma\Api\Data\MessageInterface $message, $notify = true)
    {
        try {
            ...
            if (!$message->isManager() && !$message->isSystem() && $notify) {
                $this->eventManager->dispatch(
                    \Amasty\Rma\Observer\RmaEventNames::NEW_CHAT_MESSAGE,
                    ['request' => $this->requestRepository->getById($message->getRequestId())]
                );
            }
