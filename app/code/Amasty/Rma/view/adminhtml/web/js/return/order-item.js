define([
    ...
    return Element.extend({
        ...
        whoPays: function (reason_id) {
            var payer = '';
            if (reason_id !== '') {
                _.each(this.containers[0].reasons, function (reason) {
                    if (reason.value == reason_id) {
                        if (reason.payer == 1) {
                            payer = $.mage.__('Store is supposed to cover shipping costs');
                        } else {
                            payer = $.mage.__('Customer is supposed to cover shipping costs');
                        }
                    }
                });
            }
