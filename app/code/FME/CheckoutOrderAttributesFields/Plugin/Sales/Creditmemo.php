...
class Creditmemo extends \Magento\Sales\Model\Order\Pdf\Creditmemo
{
    ...
    public function aroundGetPdf(
        ...
        $subject->_initRenderer('creditmemo');
        ...
        foreach ($creditmemos as $creditmemo) {
            if ($creditmemo->getStoreId()) {
                $subject->_localeResolver->emulate($creditmemo->getStoreId());
                $subject->_storeManager->setCurrentStore($creditmemo->getStoreId());
            }
            $page = $subject->newPage();
            $order = $creditmemo->getOrder();
            /* Add image */
            $subject->insertLogo($page, $creditmemo->getStore());
            /* Add address */
            $subject->insertAddress($page, $creditmemo->getStore());
            /* Add head */
            $subject->insertOrder(
                $page,
                $order,
            ...
            $subject->insertDocumentNumber($page, __('Credit Memo # ') . $creditmemo->getIncrementId());
            /* Add table head */
            ...
            /* Add totals */
            $subject->insertTotals($page, $creditmemo);
        }
