    ...
    public function aroundGetPdf(
        ...
        foreach ($invoices as $invoice) {
            if ($invoice->getStoreId()) {
                $subject->_localeResolver->emulate($invoice->getStoreId());
                $subject->_storeManager->setCurrentStore($invoice->getStoreId());
            }
            ...
            /* Add image */
            $subject->insertLogo($page, $invoice->getStore());
            /* Add address */
            $subject->insertAddress($page, $invoice->getStore());
            /* Add head */
            $subject->insertOrder(
                $page,
                $order,
            ...
            $subject->insertDocumentNumber($page, __('Invoice # ') . $invoice->getIncrementId());
            /* Add table */
            ...
            /* Add totals */
            $subject->insertTotals($page, $invoice);
            if ($invoice->getStoreId()) {
