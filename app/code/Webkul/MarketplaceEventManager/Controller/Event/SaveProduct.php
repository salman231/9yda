...
class SaveProduct
{
    ...
    public function saveProductData($sellerId, $wholedata)
    {
        ...
        $tempqty= $wholedata['product']['qty'];  
        $wholedata['product']['quantity_and_stock_status'] = ['qty'=>$tempqty, 'is_in_stock'=>($tempqty?1:0)];

        $helper = $this->_marketplaceHelperData;

        $sellerId = $sellerId;
        $optionDataValues = $wholedata['product'];
        if (!empty($wholedata['id'])) {
            ...
            unset($wholedata['product']['options']);
        ...
        */
        ...
        if (isset($wholedata[$catalogProduct->getIdFieldName()])) {
            throw new \Magento\Framework\Exception\LocalizedException(
                __('Unable to save event')
            );
        }

        $originalSku = $catalogProduct->getSku();
        $catalogProduct->setEventStartDate($wholedata['product']['event_start_date']);
        $catalogProduct->setEventEndDate($wholedata['product']['event_end_date']);
        ...
        */
        $this->updateQuantity($wholedata, $optionDataValues);

        /*
        * Marketplace Product Send Mail Function
        */
        if ($editFlag) {
            $this->saveOptionValues($optionDataValues);
            $catalogProduct->save();
        } else {
            $catalogProduct->setData('is_salable', 1)->save();
        }
        $this->sendProductMail($wholedata, $sellerId, $editFlag);

        $returnArr['product_id'] = $mageProductId;
        $data = ['has_options' => 1, 'required_options' => 1];
        $productentity = $this->_objectManager->create(
            'Webkul\MarketplaceEventManager\Model\Product'
        )->load($mageProductId);
        $productentity->addData($data)->setId($mageProductId)->save();

        return $returnArr;
    }

    public function saveOptionValues($value)
    {
        if (isset($value[0]['option_id'])) {
            $this->option->load($value[0]['option_id'])->setData($value[0])->save();
        } else {
            $this->option->setData($value[0])->save();
        }
    }

    protected function updateQuantity($wholedata, $optionDataValues)
    {
        $id = $wholedata['id'];
        $productModel = $this->_product->create()->load($id);
        if ($productModel->getTypeId() == 'etickets') {
           $qty= $wholedata['product']['qty'];
            $productModel->setStockData(
                [
                    'use_config_manage_stock' => $wholedata['product']['stock_data']['use_config_manage_stock'],
                    'manage_stock'=>1,
    ...
    private function productInitialize(\Magento\Catalog\Model\Product $catalogProduct, $requestData)
    {
        ...
        */
        ...
        if (empty($requestData['affect_product_custom_options'])) {
            $requestData['affect_product_custom_options'] = 1;
        }
    ...
    private function sendProductMail($data, $sellerId, $editFlag = null)
    {
        ...
        if ($editFlag == null) {
            $emailTempVariables['myvar4'] = __(
                'I would like to inform you that recently I have added a new coupon in the store.'
            );
        } else {
            $emailTempVariables['myvar4'] = __(
                'I would like to inform you that recently I have updated a coupon in the store.'
            );
        }
        $senderInfo = [
            'name' => $sellerName,
