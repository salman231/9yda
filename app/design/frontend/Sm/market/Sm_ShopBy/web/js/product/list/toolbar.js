...
define([
    ...
    $.widget('mage.productListToolbarForm', $.mage.productListToolbarForm, {
        ...
        _create: function () {
            ...
            ;
            var listItems = $(".filter-options ol li");
            $.each( listItems, function( index, value ){
                var $product = $(this);
                if(!$product.hasClass('level-1') && (!$product.hasClass('level-2'))) {
                    var test_str = $product.find('a').clone().children().remove().end().text();
                    if (test_str.indexOf('19') >= 0) {
                        $product.addClass('level-1');
                    } else {
                        $product.addClass('level-2');
                    }
                    var new_str = test_str.replace(/[0-9]/g, '');
                    $product.find('a').text(new_str);
                }
            });
            $('.filter-options ol li.level-2').css('padding-left', '20px');
            $('.filter-options ol li.level-2').hide();
            $(".filter-options-title .cust_span").eq(1).hide();
        ...
        processAjaxCall: function (baseUrl, paramData) {
            ...
            $.ajax({
                ...
                $('.filter-options ol li.level-2').css('padding-left', '20px');
                $('.filter-options ol li.level-2').show();
                $(".filter-options-title .cust_span").eq(1).hide();
